...
GLOBAL_LIST_INIT(character_flaws, list(
	...
	"Wild Magic"=/datum/charflaw/wildmagic,
...
/datum/charflaw/greedy
	name = "Greedy"
	desc = "I can't get enough sunmarks, I need more and more! I've also become good at knowing how much things are worth."
	var/last_checked_sunmarks = 0
	var/required_sunmarks = 0
	var/next_sunmark_increase = 0
	var/last_passed_check = 0
	var/first_tick = FALSE
	var/extra_increment_value = 0

/datum/charflaw/greedy/on_mob_creation(mob/user)
	next_sunmark_increase = world.time + rand(15 MINUTES, 25 MINUTES)
	last_passed_check = world.time
	ADD_TRAIT(user, TRAIT_SEEPRICES_SHITTY, "[type]")

/datum/charflaw/greedy/flaw_on_life(mob/user)
	if(!first_tick)
		determine_starting_sunmarks(user)
		first_tick = TRUE
		return
	if(world.time >= next_sunmark_increase)
		sunmark_increase(user)
	sunmark_check(user)

/datum/charflaw/greedy/proc/determine_starting_sunmarks(mob/living/carbon/human/user)
	var/starting_sunmarks = get_sunmarks_in_atom(user)
	required_sunmarks = round(starting_sunmarks * 0.7)
	extra_increment_value = round(starting_sunmarks * 0.15)

/datum/charflaw/greedy/proc/sunmark_increase(mob/living/carbon/human/user)
	if(last_passed_check + (50 MINUTES) < world.time) //If we spend a REALLY long time without being able to satisfy, then pity downgrade
		required_sunmarks -= rand(10, 20)
		to_chat(user, span_blue("Maybe a little less sunmarks is enough..."))
	else
		required_sunmarks += rand(25, 35) + extra_increment_value
	required_sunmarks = min(required_sunmarks, 250) //Cap at 250 coins maximum
	next_sunmark_increase = world.time + rand(35 MINUTES, 40 MINUTES)
	var/current_sunmarks = get_sunmarks_in_atom(user)
	if(current_sunmarks >= required_sunmarks)
		to_chat(user, span_blue("I'm quite happy with the amount of sunmarks I have..."))
	else
		to_chat(user, span_boldwarning("I need more sunmarks, what I have is not enough..."))

	last_checked_sunmarks = current_sunmarks

/datum/charflaw/greedy/proc/sunmark_check(mob/living/carbon/human/user)
	var/new_sunmark_amount = get_sunmarks_in_atom(user)
	var/ascending = (new_sunmark_amount > last_checked_sunmarks)

	var/do_update_msg = TRUE
	if(new_sunmark_amount >= required_sunmarks)
		// Feel better
		if(user.has_stress_event(/datum/stressevent/vice))
			to_chat(user, span_blue("[new_sunmark_amount] sunmarks... That's more like it.."))
	...
	if(new_sunmark_amount == last_checked_sunmarks)
		do_update_msg = FALSE

	if(do_update_msg)
		if(ascending)
			to_chat(user, span_warning("Only [new_sunmark_amount] sunmarks.. I need more..."))
		else
			to_chat(user, span_boldwarning("No! My precious sunmarks..."))

	last_checked_sunmarks = new_sunmark_amount
